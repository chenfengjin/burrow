...
func TestEVM(t *testing.T) {
	...
	t.Run("SendCall", func(t *testing.T) {
		...
		txe := runVM(st, account1, account2, contractCode, 19)
	...
	t.Run("DelegateCallGas", func(t *testing.T) {
		...
		delegateCallCost := baseOpsCost + pushCost + returnCost
	...
	t.Run("SIGNEXTEND and MSTORE8", func(t *testing.T) {
		st := acmstate.NewMemoryState()
		blockchain := new(engine.TestBlockchain)
		eventSink := exec.NewNoopEventSink()
...
func call(vm *EVM, st acmstate.ReaderWriter, origin, callee crypto.Address, code []byte, input []byte,
	...
	evs := new(exec.Events)
	out, err := vm.Execute(st, new(engine.TestBlockchain), evs, engine.CallParams{
		Caller: origin,
		Callee: callee,
		Input:  input,
		Gas:    gas,
...
func runVM(st acmstate.ReaderWriter, caller, callee crypto.Address, code []byte, gas uint64) *exec.TxExecution {
	...
	output, err := vm.Execute(st, new(engine.TestBlockchain), txe, params, code)
	txe.PushError(err)
	for _, ev := range txe.ExceptionalCalls() {
		txe.PushError(ev.Header.Exception)
	}
	txe.Return(output, gasBefore.Sub(gasBefore, bigGas).Uint64())
	return txe
}
